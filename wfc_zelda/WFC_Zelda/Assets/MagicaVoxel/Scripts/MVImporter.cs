...
public static class MVImporter 
{
		...
		for (int x = 0; x < voxelChunk.sizeX; ++x) {
			for (int y = 0; y < voxelChunk.sizeY; ++y) {
				for (int z = 0; z < voxelChunk.sizeZ; ++z) {
					...
					// up downCreateIndividualVoxelGameObjects					if (y == 0 || voxelChunk.voxels[x, y - 1, z] == 0)
    ...
    for (int x = 39; x<=47; x++) {
      GameObject.Find("Voxel (39, 54, " + x + ")").SetActive(false);
      GameObject.Find("Voxel (40, 54, "+x+")").SetActive(false);
      GameObject.Find("Voxel (41, 54, " + x + ")").SetActive(false);
      GameObject.Find("Voxel (42, 54, " + x + ")").SetActive(false);
      GameObject.Find("Voxel (43, 54, " + x + ")").SetActive(false);
      GameObject.Find("Voxel (44, 54, " + x + ")").SetActive(false);
      GameObject.Find("Voxel (45, 54, " + x + ")").SetActive(false);
      GameObject.Find("Voxel (46, 54, " + x + ")").SetActive(false);
      GameObject.Find("Voxel (47, 54, " + x + ")").SetActive(false);

    }
   

    GameObject thePlayer = GameObject.Find("vox");
    MVVoxModel model = thePlayer.GetComponent<MVVoxModel>();
    //CreateNew(model.par, "Assets/" + parent.name + ".prefab");
